...
class OfflineManager():
    ...
    def run_database_threading(self, bundle_liste, tree, discarded_tree):
        ...
        worker.signals.progress.connect(self.progress_fn, Qt.DirectConnection) # signal to update progress bar
        worker.signals.result.connect(self.close_database,Qt.DirectConnection)
        worker.signals.finished.connect(partial(self.tree_view_manager.update_treeview,tree,discarded_tree)) # when done, update the treeview
        self.threadpool.start(worker) # start the thread
        self.bundle_liste = []


    def close_database(self, result):
        result.close()
        connection = duckdb.connect("C:/Users/user/Dropbox/Biophysical_Essentials/src/duck_db_analysis_database.db")
        print(connection)
